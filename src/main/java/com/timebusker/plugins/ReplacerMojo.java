...
import com.timebusker.plugins.file.FileUtils;
import com.timebusker.plugins.include.FileSelector;
...
public class ReplacerMojo extends AbstractMojo {
	...
	private void replaceContents(ReplacementProcessor processor, List<Replacement> replacements, String inputFile) throws IOException {
		...
		try {
			processor.replace(replacements, regex, getBaseDirPrefixedFilename(inputFile), outputFileName, patternFlagsFactory.buildFlags(regexFlags), encoding);
	...
	private List<Replacement> buildReplacements() throws IOException {
		...
		if (variableTokenValueMap != null) {
			return tokenValueMapFactory.replacementsForVariable(variableTokenValueMap, isCommentsEnabled(), unescape, encoding);
		}
